...
import { ArrowLeft, FileArrowUp, FileText, Graph, SquaresFour } from "@phosphor-icons/react";
...
export function Workspace({ initialCourse, initialDocuments, graph }: { initialCourse: Course; initialDocuments: CourseDocument[]; graph: CourseGraph }) {
  ...
  return <main className="min-h-[100dvh]">
    ...
    <div className="mx-auto max-w-[1280px] px-5 py-8 lg:px-8 lg:py-10">
      ...
      {tab === "sources" && <section className="py-8"><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-semibold tracking-[-0.03em]">Course sources</h2><p className="mt-1 text-sm text-[var(--muted)]">PDF, Markdown, text, or Word. Up to 25 MB.</p></div><button onClick={() => setUploadOpen((value) => !value)} className="focus-ring inline-flex h-10 items-center gap-2 bg-[var(--ink)] px-4 text-sm font-medium text-white"><FileArrowUp size={17} /> Upload source</button></div>
      ...
      {tab === "graph" && <section className="py-8"><div className="mb-6"><h2 className="text-xl font-semibold tracking-[-0.03em]">Course graph</h2><p className="mt-1 text-sm text-[var(--muted)]">Drag nodes and use the controls to inspect relationships.</p></div>{graph.nodes.length ? <GraphView graph={graph} /> : <div className="grid min-h-[440px] place-items-center border border-dashed border-[#bec8c1] bg-white p-8 text-center"><div><Graph size={34} className="mx-auto mb-4 text-[var(--accent)]" /><h3 className="font-semibold">No course graph yet.</h3><p className="mt-2 max-w-sm text-sm leading-6 text-[var(--muted)]">Arc will build connections between concepts, lectures, assignments, and source material here.</p></div></div>}</section>}
    </div>
  </main>;
}
