...
import type { CandidateQueue, Course, CourseDocument, CourseGraph } from "@arc/shared";
import { DOCUMENT_TYPES } from "@arc/shared";
import { ArrowLeft, ClipboardText, FileArrowUp, FileText, Graph, SquaresFour } from "@phosphor-icons/react";
...
export function Workspace({ initialCourse, initialDocuments, initialGraph, initialQueue }: { initialCourse: Course; initialDocuments: CourseDocument[]; initialGraph: CourseGraph; initialQueue: CandidateQueue }) {
  ...
  return <main className="min-h-[100dvh]">
    <header className="border-b border-[var(--line)] bg-white"><div className="mx-auto flex h-16 max-w-[1280px] items-center justify-between px-5 lg:px-8"><Brand /><Link href="/" className="focus-ring inline-flex items-center gap-2 text-sm text-[var(--muted)] hover:text-[var(--ink)]"><ArrowLeft size={16} /> All courses</Link></div></header>
    <div className="mx-auto max-w-[1280px] px-5 py-8 lg:px-8 lg:py-10">
      ...
      <nav aria-label="Course sections" className="flex gap-7 border-b border-[var(--line)]">{([{ id: "overview", label: "Overview", icon: SquaresFour }, { id: "sources", label: "Sources", icon: FileText }, { id: "graph", label: "Graph", icon: Graph }, { id: "review", label: "Review", icon: ClipboardText }] as const).map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} aria-current={tab === id ? "page" : undefined} className={`focus-ring -mb-px inline-flex h-12 items-center gap-2 border-b-2 text-sm font-medium ${tab === id ? "border-[var(--accent)] text-[var(--ink)]" : "border-transparent text-[var(--muted)] hover:text-[var(--ink)]"}`}><Icon size={17} />{label}{id === "review" && pendingCount > 0 && <span className="border border-[#9bc8b4] bg-[#eef8f3] px-1.5 font-mono text-[10px] text-[#075b40]">{pendingCount}</span>}</button>)}</nav>
      ...
      {tab === "review" && <GraphReview courseId={initialCourse.id} initialQueue={initialQueue} approvedNodes={graph.nodes} onReviewed={handleReviewed} />}
    </div>
  </main>;
}
